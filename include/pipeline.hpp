...
VkRenderPass create_render_pass(Instance *instance)
{
    ...
    */
    ...
    if(vkCreateRenderPass(instance->device, &ci, nullptr, &render_pass) == VK_SUCCESS){
...
public:
    ...
    void init(Instance *instance, Descriptors *descriptors, VkRenderPass *render_pass)
    {
    ...
    void create_graphics_pipeline()
    {
        ...
        //std::vector<char> vertShaderCode = read_file("shaders/vert-model.spv");
        //std::vector<char> fragShaderCode = read_file("shaders/frag-model.spv");
    ...
    {
        std::vector<char> vertShaderCode = read_file("shaders/vert-skybox.spv");
        std::vector<char> fragShaderCode = read_file("shaders/frag-skybox.spv");
        ...
        depthStencil.depthTestEnable = VK_FALSE; // compare depth of new frags with depth buffer if they should be discarded
        depthStencil.depthWriteEnable = VK_FALSE; // if they pass depth test, write to depth buffer
        depthStencil.depthCompareOp = VK_COMPARE_OP_LESS;
        depthStencil.depthBoundsTestEnable = VK_FALSE;
        depthStencil.minDepthBounds = 0.0f; // Optional
        depthStencil.maxDepthBounds = 1.0f; // Optional
